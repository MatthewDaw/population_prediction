...
from experiments.experiment_sdk_v2.submodels.experiment_parameters import DataTransformationOptions, AlwaysOrNeverOperationChoices, OperationChoices
...
class DataTransformer:
    ...
    def normalize_data(self, df, data_transformation_options: DataTransformationOptions):
        """
        ...
        if data_transformation_options.drop_near_constant_columns == AlwaysOrNeverOperationChoices.always:
            numique = df.nunique()
        ...
        for col in columns_to_include:
            ...
            if (data_transformation_options.difference == OperationChoices.conditional and skew_val > 1) or data_transformation_options.log == AlwaysOrNeverOperationChoices.always:
                rules["log"] = True
            ...
            p_value = result[1]
            if (data_transformation_options.difference == OperationChoices.conditional and p_value > 0.05) or data_transformation_options.difference == AlwaysOrNeverOperationChoices.always:
            ...
            if data_transformation_options.z_normalize == AlwaysOrNeverOperationChoices.always:
        ...
        self.restorative_values = RestorativeValues(
            ...
            remaining_column_names=list(transformed_df.columns)
        )
        return transformed_df

    def undo_transformation_for_forcast(self, forecast):
        """Undo the transformations for forecasted values."""
        forecasted_values = pd.DataFrame(forecast, columns=self.restorative_values.remaining_column_names)
        corrected_forecasted_values = self.undo_transformations(forecasted_values, add_first_row=False)
        return corrected_forecasted_values
