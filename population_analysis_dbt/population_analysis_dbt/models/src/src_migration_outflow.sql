...
grouped_countyoutflow1718 as (
SELECT
    2017 as start_year,
    2018 as end_year,
    y2_state as state_name,
    sum(n2) as n2
FROM {{ ref('countyoutflow1718') }} group by y2_state
),
...
state_out_flow_17_18 AS (
select 
    2017 as start_year,
    2018 as end_year,
    second_el.state_name,
    (first_el.number_of_individuals + second_el.number_of_individuals) / 2 as number_of_individuals
from state_out_flow_16_17 as first_el
join 
state_out_flow_18_19 as second_el 
ON first_el.state_name = second_el.state_name
 ),
...
UNION ALL
select * from state_out_flow_17_18
UNION ALL
